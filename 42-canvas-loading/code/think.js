...
function renderCircle(){
  ...
  ctx.beginPath();
  ctx.arc(canvas.width/2, canvas.height/2, R, -Math.PI/2, angle-Math.PI/2, false);
  ctx.stroke();
  ctx.restore();
}
function renderWave(){
  ...
  ctx.save();
  setCircleClip();
  drawWave(6,-2*Math.PI/100,curValue/4,'rgba(34,167,250,0.9)');
  drawWave(8,2*Math.PI/100,curValue/4,'rgba(0,0,255,0.5)');
  ctx.restore();
}

function setCircleClip(){
  ctx.beginPath();
  ctx.arc(canvas.width/2, canvas.height/2, R-circleThick/3*2,0,Math.PI*2, false);
  ctx.clip();
}
