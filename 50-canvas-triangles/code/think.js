...
function Point(r,c){
  ...
  this.createPos = function(){
    return {
      x:
      (this.col-3)*size + Math.random()*size*randomRatio,
      y:
      (this.row-3)*size + Math.random()*size*randomRatio
    };
  };
  this.render = function(time){
    ...
    if(dur>=this.totalTime){
      ...
      this.x
      =this.startX + (this.endX - this.startX)
          *dur/this.totalTime;
      this.y
      =this.startY + (this.endY - this.startY)
          *dur/this.totalTime;
    }
  };
...
function Triangle(p1,p2,p3){
  ...
  this.endBright = 60+Math.round(Math.random() * 20);
  this.startTime;
  this.totalTime;
  ...
  this.reset = function(){
    ...
    this.endHue = 250+Math.round(Math.random() * 20);
    this.endBright = 60+Math.round(Math.random() * 25);
  };
  this.reset();
  this.draw = function(){
    var color;
    ...
    ctx.closePath();
    color="hsla("+this.hue+",50%,"+this.bright+"%,0.5)";
